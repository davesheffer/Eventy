...
import { useEditEvent, useFetchEvent } from '../../queries/events';
import { useFetchEvent } from '../../queries/events';
import moment from 'moment';
import classNames from 'classnames';
import { AiFillCloseSquare } from 'react-icons/ai';
import GlobalContext from '../../context/GlobalContext';

const EditEvent = ({ events, setEvents, menuToggle, setMenuToggle }) => {
    const { categories, locations } = useContext(GlobalContext);
    const { data: editedEvent } = useFetchEvent();
    const [location, setLocation] = useState('');
    const [createdAt, setCreatedAt] = useState(moment().format('yyyy-MM-DD'));
    const [category, setCategory] = useState('');
    const [timing, setTiming] = useState('');
    const { mutate: editEvent } = useAddEvent();
